...
public:
    ...
    void display() {
        ...
        std::cout << "strategy = " << ((int) algorithm) << std::endl;
        std::cout << "in_db_file_path = " << dataset << std::endl;
...
void load_data(ExpConfig &config, RowStore &table) {
    duckdb::DuckDB db(config.dataset);
...
int main(int argc, char *argv[]) {
    ...
    config.algorithm = Strategy::SEQUENTIAL;
    config.dataset = "data/tpch-sf1.db";
    ...
    app.add_option("--in_db_file_path", config.dataset);
    ...
    if (strat_str == "SEQUENTIAL") {
        ...
        config.algorithm = Strategy::GLOBAL_LOCK;
    } else if (strat_str == "TWO_PHASE_CENTRALIZED_MERGE") {
        config.algorithm = Strategy::TWO_PHASE_CENTRALIZED_MERGE;
    } else if (strat_str == "SIMPLE_TWO_PHASE_RADIX") {
        config.algorithm = Strategy::SIMPLE_TWO_PHASE_RADIX;
    } else if (strat_str == "SIMPLE_THREE_PHASE_RADIX") {
        config.algorithm = Strategy::SIMPLE_THREE_PHASE_RADIX;
    } else if (strat_str == "IMPLICIT_REPARTITIONING") {
        config.algorithm = Strategy::IMPLICIT_REPARTITIONING;
    } else if (strat_str == "DUCKDBISH_TWO_PHASE") {
        config.algorithm = Strategy::DUCKDBISH_TWO_PHASE;
    } else {
    ...
    auto run_once = [&](int run_idx, std::vector<AggResRow>& agg_res) {
        if (config.algorithm == Strategy::SEQUENTIAL) {
            sequential_sol(config, table, run_idx, agg_res);
        } else if (config.algorithm == Strategy::GLOBAL_LOCK) {
            dumb_global_lock_sol(config, table, run_idx, agg_res);
        } else if (config.algorithm == Strategy::TWO_PHASE_CENTRALIZED_MERGE) {
            naive_2phase_centralised_merge_sol(config, table, run_idx, agg_res);
        } else if (config.algorithm == Strategy::SIMPLE_TWO_PHASE_RADIX) {
            simple_2phase_radix_partition_sol(config, table, run_idx, agg_res);
        } else if (config.algorithm == Strategy::SIMPLE_THREE_PHASE_RADIX) {
            simple_3phase_radix_partition_sol(config, table, run_idx, agg_res);
        } else if (config.algorithm == Strategy::IMPLICIT_REPARTITIONING) {
            implicit_repartitioning_sol(config, table, run_idx, agg_res);
        } else if (config.algorithm == Strategy::DUCKDBISH_TWO_PHASE) {
